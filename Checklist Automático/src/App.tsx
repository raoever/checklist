...
const ITENS_PADRAO = [
  ...
  [
    "II",
    "Folhas de ponto (com relatório de horas, se aplicável)",
  ],
  ...
  [
    "XIII",
    "Apólice de Seguro de Vida (com certificados individuais)",
  ],
  [
    "01",
...
const ITENS_DEMISSAO = [
  [
    "a)",
    "Aviso prévio / Comunicado de Dispensa / Pedido de Demissão",
  ],
  [
    "b)",
  ...
  [
    "g)",
    "Comprovante de pagamento de multa de 40% do FGTS, se for o caso",
  ],
  ["h)", "Exame demissional"],
  ["i)", "Folha de Ponto"],
...
const getItemSortOrder = (itemCode: string): number => {
  ...
  if (demissaoMatch)
    return 300 + demissaoMatch[1].charCodeAt(0) - 97;
...
const validateReportFields = (
  ...
  checklistItems.forEach((item) => {
    if (
      ...
      errors.push(
        `Informe a observação para o item [${item.item}] - ${item.descricao.substring(0, 30)}...`,
      );
    }
  });
  ...
  const itemsWithMissingSEI = checklistItems.filter(
    (item) =>
      !item.funcionarioId && !item.extras.numero_sei?.trim(),
  );
...
const gerarRelatorio = (
  ...
  const standardItems = itemsToShow.filter(
  ...
  const generateSignatureSection = () => {
    ...
    return `
      <div class="footer avoid-break">
        ...
        <p><em>${cargo}</em></p>
        <p>Palmas-TO, ${formatDateForDisplay(companyData.dataConferencia) || getCurrentDateExtended()}</p>
        <br>
  ...
  const htmlContent = `
    ...
    <body>
      <div class="container">
        ...
        <h1>
          CONFERÊNCIA DOS DOCUMENTOS TRABALHISTAS ${tipagem === "pendencias" ? "- PENDÊNCIAS" : "- COMPLETO"}
        </h1>
        
        <div class="company-info avoid-break">
          <div><strong>Processo:</strong> ${companyData.numeroProcesso || "Não informado"}</div>
          <div><strong>Contrato:</strong> ${companyData.numeroContrato || "Não informado"}</div>
          <div><strong>Empresa:</strong> ${companyData.empresa || "Não informado"}</div>
          <div><strong>Medição - Período:</strong> ${companyData.numeroMedicao || "Não informado"} - ${companyData.periodo || "Período não informado"}</div>
          ${companyData.conferencia ? `<div><strong>Conferência:</strong> ${companyData.conferencia}</div>` : ""}
          ${companyData.mesReferencia ? `<div><strong>Mês/Ano de Referência da MP:</strong> ${companyData.mesReferencia}</div>` : ""}
          ${companyData.dataConferencia ? `<div><strong>Data da Conferência:</strong> ${formatDateForDisplay(companyData.dataConferencia)}</div>` : ""}
        </div>

        <div class="content-section">
          <strong>1.</strong> Considerando protocolo SEI ${companyData.protocoloSEI || "Não informado"} no Processo SEI ${companyData.numeroProcesso || "Não informado"}.
        </div>
...
export default function App() {
  ...
  const [checklistItems, setChecklistItems] = useState<
    ChecklistItem[]
  >([]);
  const [funcionariosDemissao, setFuncionariosDemissao] =
    useState<FuncionarioDemissao[]>([]);
  const [funcionariosAdmissao, setFuncionariosAdmissao] =
    useState<FuncionarioAdmissao[]>([]);
  const [
    funcionariosTransferencia,
    setFuncionariosTransferencia,
  ] = useState<FuncionarioTransferencia[]>([]);
  ...
  const [showDemissaoDialog, setShowDemissaoDialog] =
    useState(false);
  ...
  const [showAdmissaoDialog, setShowAdmissaoDialog] =
    useState(false);
  ...
  const [showTransferenciaDialog, setShowTransferenciaDialog] =
    useState(false);
    ...
    if (saved) {
      setTimeout(() => {
        ...
        if (saldosCount > 1) {
          ...
          toast.info(
            "Dados carregados - Status dos itens preservados",
          );
        }
      }, 1500);
    }
  ...
  useEffect(() => {
    ...
    setChecklistItems((prev) => {
      ...
      const standardItems = cleanItems.filter(
        (item) => !item.funcionarioId,
      );
      ...
      const missingItems = ITENS_PADRAO.filter(
        ([code, desc]) =>
          !existingItems.has(createItemKey(code, desc)),
      );
  ...
  const updateLocalData = () => {
    ...
    const dataConferencia =
      companyData.dataConferencia || getCurrentDate();
    const dateText =
      formatDateForDisplay(dataConferencia) ||
      ...
      if (saved) {
        ...
        if (data.meta) {
          setCompanyData((prev) => ({
            ...
            dataConferencia:
              data.meta.dataConferencia || getCurrentDate(),
        ...
        if (data.itens) {
          ...
          let cleanItems = removeDuplicateItems(
            itemsWithUniqueIds,
          );
          cleanItems = cleanSaldoEmpenhoItems(cleanItems);
        ...
        if (data.funcionariosTransferencia) {
          setFuncionariosTransferencia(
            data.funcionariosTransferencia,
          );
        }
      }
    } catch (error) {
  ...
  const handleExtraFieldBlur = (
    ...
    if (config?.currency && value.trim()) {
      ...
      if (processedValue && !validateMMAAAA(processedValue)) {
        toast.error(
          "Formato inválido. Use MM/AAAA (ex.: 05/2025)",
        );
        return;
      }
    }
  ...
  const adicionarFuncionarioDemitido = () => {
    ...
    ITENS_DEMISSAO.forEach(([code, desc], index) => {
      newItems.push({
        ...
        extras: {
          ...
          descricao_documento:
            demissaoForm.descricaoDocumento || "",
        },
        sortOrder: Date.now() + index,
    ...
    toast.success(
      `Itens de demissão adicionados para ${demissaoForm.nome}`,
    );
  };
  ...
  const adicionarFuncionarioAdmitido = () => {
    ...
    ITENS_ADMISSAO.forEach(([code, desc], index) => {
      newItems.push({
        ...
        extras: {
          ...
          descricao_documento:
            admissaoForm.descricaoDocumento || "",
        },
        sortOrder: Date.now() + index,
        funcionarioId: funcionarioId,
        lembrete: "",
    ...
    toast.success(
      `Itens de admissão adicionados para ${admissaoForm.nome}`,
    );
  };
  ...
  const removeFuncionarioTransferido = (
    funcionarioId: string,
  ) => {
    setFuncionariosTransferencia((prev) =>
      prev.filter((f) => f.id !== funcionarioId),
  ...
  const limparChecklist = () => {
    ...
    const itensPadrao = checklistItems.filter(
      (item) => !item.funcionarioId,
    );
    ...
    itensFuncionarios.forEach((item) =>
  ...
  const removerDuplicatas = () => {
    ...
    if (duplicatesRemoved > 0) {
      ...
      setChecklistItems(cleanItems);
      toast.success(
        "Itens verificados e saldos de empenho corrigidos",
      );
    }
  };
  ...
  const generateFileName = (): string => {
    const conferencia =
      companyData.conferencia?.trim() || "SemConferencia";
    const medicao =
      companyData.numeroMedicao?.trim() || "SemMedicao";
    const contrato =
      companyData.numeroContrato?.trim() || "SemContrato";
    ...
    return `${clean(conferencia)}_${clean(medicao)}_${clean(contrato)}_${clean(empresa)}.json`;
  };
  ...
  const exportarJSONPendenciasProximaMP = () => {
    ...
    const funcionariosDemissaoComPendencias =
      funcionariosDemissao.filter((funcionario) =>
        checklistItems.some(
          (item) =>
    ...
    const funcionariosAdmissaoComPendencias =
      funcionariosAdmissao.filter((funcionario) =>
        checklistItems.some(
          (item) =>
            item.funcionarioId === funcionario.id &&
            item.status === "PENDENCIA_PROXIMA_MP",
    ...
    const data = {
      ...
      funcionariosTransferencia:
        funcionariosTransferenciaComPendencias,
    };
    ...
    const generatePendenciasFileName = (): string => {
      ...
      const contrato =
        companyData.numeroContrato?.trim() || "SemContrato";
      const empresa =
        companyData.empresa?.trim() || "SemEmpresa";

      // Remove caracteres especiais e espaços
      const clean = (str: string) =>
        str.replace(/[^\w\-]/g, "_").replace(/_{2,}/g, "_");

      return `PENDENCIAS_PROXIMA_MP_${clean(conferencia)}_${clean(medicao)}_${clean(contrato)}_${clean(empresa)}.json`;
    };
        ...
        if (data.meta) {
          ...
          if (isExportacaoPendencias) {
            ...
            setCompanyData((prev) => ({
              ...
              numeroProcesso:
                prev.numeroProcesso ||
                data.meta.numeroProcesso ||
                "",
              numeroContrato:
                prev.numeroContrato ||
                data.meta.numeroContrato ||
                "",
              numeroMedicao:
                prev.numeroMedicao ||
                data.meta.numeroMedicao ||
                "",
          ...
          if (isExportacaoPendencias) {
            ...
            setFuncionariosDemissao((prev) => {
              const existingIds = new Set(
                prev.map(
                  (f) =>
                    f.nome + f.dataAdmissao + f.dataDemissao,
                ),
              );
          ...
          if (isExportacaoPendencias) {
            ...
            setFuncionariosAdmissao((prev) => {
              ...
              const newFuncionarios =
                data.funcionariosAdmissao.filter(
        ...
        if (data.funcionariosTransferencia) {
          if (isExportacaoPendencias) {
            ...
            setFuncionariosTransferencia((prev) => {
              ...
              const newFuncionarios =
                data.funcionariosTransferencia.filter(
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                className={`text-sm ${!item.extras.numero_sei?.trim() ? "border-red-500 bg-red-50" : ""}`}
                placeholder="Número do SEI"
              />
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-gray-500">
                💡 Aceita formatos: 1.500,50 (brasileiro),
                1500,50 (simplificado) ou 1500.50 (americano).
              </p>
            </div>
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <Input
                  ...
                  className={`text-sm ${hasError ? "border-red-500 bg-red-50" : ""}`}
                  placeholder={
                    field.id === "numero_sei"
                      ? "Número do SEI"
                      : field.currency
                ...
                {field.currency && (
                  <p className="text-xs text-gray-500">
                    💡 Aceita formatos: 1.500,50 (brasileiro),
                    1500,50 (simplificado) ou 1500.50
                    (americano). A formatação será aplicada
                    automaticamente.
                  </p>
                )}
                {field.mmAAAA && (
                  <p className="text-xs text-gray-500">
                    💡 Digite livremente. Será formatado para
                    MM/AAAA automaticamente.
                  </p>
                )}
              </div>
            );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="50623.001286/2025-58"
                  className={`placeholder-gray-400 ${getFieldValidationClass("numeroProcesso", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="ETHOS"
                  className={`placeholder-gray-400 ${getFieldValidationClass("empresa", companyData)}`}
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="numeroMedicao">
                  Nº da Medição *
                </Label>
                <Input
                  id="numeroMedicao"
                  ...
                  placeholder="10ª MP"
                  className={`placeholder-gray-400 ${getFieldValidationClass("numeroMedicao", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={`placeholder-gray-400 ${getFieldValidationClass("numeroContrato", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="01ª conferência"
                  className={`placeholder-gray-400 ${getFieldValidationClass("conferencia", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("periodo", e.target.value)
                  }
                  placeholder="01/01/2025 a 31/01/2025"
                  className={`placeholder-gray-400 ${getFieldValidationClass("periodo", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="01/2025"
                  className={`placeholder-gray-400 ${getFieldValidationClass("mesReferencia", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={`placeholder-gray-400 ${getFieldValidationClass("dataConferencia", companyData)}`}
                  />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="protocoloSEI">
                  Protocolo SEI *
                </Label>
                <Input
                  id="protocoloSEI"
                  ...
                  className={`placeholder-gray-400 ${getFieldValidationClass("protocoloSEI", companyData)}`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleInputChange("Usuario", value)
                  }
                >
                  <SelectTrigger
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-2">
              💡 Este campo aparecerá no relatório antes da
              lista de documentos. Use para especificar
              critérios da amostragem, funcionários selecionados
              ou observações relevantes sobre a seleção.
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {checklistItems
                ...
                .map((item) => {
                  const showQuantity = ITENS_COM_QTD.has(
                    item.item.trim(),
                  );
                  const isSaldoEmpenho = item.item === "SE1";
                  const statusColorClass = getItemStatusColor(
                    item.status,
                  );
                  ...
                  return (
                    ...
                    >
                      <CollapsibleTrigger asChild>
                        <div
                          className={`flex items-center justify-between p-4 rounded-lg border-2 cursor-pointer hover:shadow-md transition-all ${statusColorClass} ${isSaldoEmpenho ? "border-l-4 border-l-yellow-500" : ""}`}
                        >
            ...
            <Card className="mt-6">
              <CardHeader>
                <CardTitle className="text-blue-700">
                  Observações (para ciência do Fiscal
                  Administrativo):
                </CardTitle>
              </CardHeader>
              <CardContent>
                ...
                />
                <p className="text-xs text-gray-500 mt-2">
                  💡 Use o botão "—" na barra de ferramentas
                  para inserir divisões e separar diferentes
                  informações.
                </p>
              </CardContent>
            </Card>
        ...
        {/* Dialogs Integrados */}
        <Dialog
          open={showDemissaoDialog}
          onOpenChange={setShowDemissaoDialog}
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                Adicionar Funcionário Demitido
              </DialogTitle>
              <DialogDescription>
                Preencha os dados do funcionário para adicionar
                todos os itens de demissão relacionados.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 mt-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="data-admissao-demissao">
                  Data de Admissão
                </Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="numero-sei-demissao">
                  Nº SEI
                </Label>
                <Input
        ...
        <Dialog
          open={showAdmissaoDialog}
          onOpenChange={setShowAdmissaoDialog}
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                Adicionar Funcionário Admitido
              </DialogTitle>
              <DialogDescription>
                Preencha os dados do funcionário para adicionar
                todos os itens de admissão relacionados.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 mt-4">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="numero-sei-admissao">
                  Nº SEI
                </Label>
                <Input
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                Adicionar Funcionário Transferido
              </DialogTitle>
              <DialogDescription>
                Preencha os dados do funcionário para adicionar
                todos os itens de transferência relacionados.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 mt-4">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="numero-sei-transferencia">
                  Nº SEI
                </Label>
                <Input
